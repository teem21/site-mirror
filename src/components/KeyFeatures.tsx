...
const KeyFeatures = () => {
  return (
    <section className="py-20 px-6 lg:px-16 border-inherit bg-primary-foreground">
      <div className="mx-auto max-w-6xl">
        <p className="mb-2 font-heading text-xs font-semibold uppercase tracking-[0.3em] text-primary">
          Exclusive Patented Design
        </p>
        <h2 className="mb-12 font-heading text-3xl font-bold tracking-tight md:text-4xl text-secondary">
          KEY CAPABILITIES
        </h2>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div
            key={i}
            className="group p-6 transition-all hover:border-glow my-0 mx-0 border border-solid rounded-none text-primary-foreground border-primary-foreground bg-primary-foreground shadow-sm"
            style={{ animationDelay: `${i * 100}ms` }}>

              <f.icon className="mb-4 h-8 w-8 text-primary transition-transform group-hover:scale-110" />
              <h3 className="mb-2 text-lg font-bold tracking-tight font-mono text-primary">
                {f.title}
              </h3>
              <p className="text-sm leading-relaxed font-mono text-secondary">
                {f.desc}
              </p>
            </div>
          )}
